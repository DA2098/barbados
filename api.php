...
try {
    ...
    if ($method === 'POST' && $action === 'create-admin') {
        ...
        if (!$admin || $admin['role'] !== 'admin') {
            ...
            echo json_encode(['error' => 'Solo un admin puede crear otro admin']);
            exit;
        }
        ...
        $created = create_user_account($pdo, $name, $email, $password, 'admin', 1, $phone);
        echo json_encode($created);
        exit;
    }

    if ($method === 'POST' && $action === 'create-barber') {
        $adminId = (string)($input['adminId'] ?? '');
        $name = trim((string)($input['name'] ?? ''));
        $email = trim((string)($input['email'] ?? ''));
        $password = (string)($input['password'] ?? '');
        ...
        if (!$admin || $admin['role'] !== 'admin') {
            http_response_code(403);
            echo json_encode(['error' => 'Solo un admin puede crear barberos']);
            exit;
        }
        ...
        $created = create_user_account($pdo, $name, $email, $password, 'barber', 1, $phone);
        echo json_encode($created);
        exit;
    }

    if ($method === 'POST' && $action === 'create-first-admin') {
        if (admin_count($pdo) > 0) {
            http_response_code(403);
            echo json_encode(['error' => 'Ya existe al menos un admin']);
            exit;
        }
    ...
    if ($method === 'POST' && $action === 'change-password' && isset($_GET['id'])) {
        $id = (string)$_GET['id'];
        ...
        $stmt = $pdo->prepare('SELECT id, name, email, role, barber_approved, phone, avatar_url, password FROM users WHERE id = ? LIMIT 1');
        $stmt->execute([$id]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if (!password_verify($currentPassword, (string)$user['password'])) {
            http_response_code(401);
            echo json_encode(['error' => 'La contraseña actual no coincide']);
            exit;
        }

        $newPasswordHash = password_hash($newPassword, PASSWORD_BCRYPT);
        $updateStmt = $pdo->prepare('UPDATE users SET password = ? WHERE id = ?');
        $updateStmt->execute([$newPasswordHash, $id]);
    ...
    if ($method === 'POST' && strpos($request_uri, '/api/barbers') !== false) {
        $data = json_decode(file_get_contents('php://input'), true);

        if (!isset($data['name'], $data['email'], $data['password'], $data['phone'], $data['barber_approved'])) {
            http_response_code(400);
            echo json_encode(["error" => "Faltan datos obligatorios"]);
            exit;
        }

        $stmt = $pdo->prepare('INSERT INTO users (name, email, password, role, phone, barber_approved) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute([
            $data['name'],
            $data['email'],
            ...
            'barber',
            $data['phone'],
            $data['barber_approved']
        ]);

        echo json_encode(["message" => "Barbero creado exitosamente"]);
        exit;
    }

    // Ruta para crear un administrador
    if ($method === 'POST' && strpos($request_uri, '/api/admins') !== false) {
        $data = json_decode(file_get_contents('php://input'), true);
        ...
        echo json_encode(["message" => "Administrador creado exitosamente"]);
        exit;
    }
