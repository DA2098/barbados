...
function walk(dir) {
  ...
  for (const file of list) {
    // skip directories listed in IGNORE
...
function check() {
  ...
  for (const f of files) {
    ...
    if (base === '.env' || base.startsWith('.env.') || base.endsWith('.example')) continue;
    // ignore README files and the scanner itself to avoid false positives
    if (base === 'readme.md') continue;
    if (path.resolve(f) === path.resolve(repoRoot, 'scripts', 'check-secrets.cjs')) continue;
    let text = '';
